  ...
  describe Profile do
    ...
    context '#append_new_work' do
      ...
      it 'should transform the input work to xml and deliver to the remote_service' do
        remote_service.should_receive(:call).with(orcid_profile_id, orcid_work.to_xml, token: token)
        mapper.should_receive(:map).with(non_orcid_work, target: 'orcid/work').and_return(orcid_work)

        subject.append_new_work(non_orcid_work, remote_service: remote_service, mapper: mapper, token: token)
      end
    end
  end
end
